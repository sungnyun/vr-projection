...
class CIFAR_Wrapper(torch.utils.data.Dataset):

    def __init__(self, root, train, download, transform, nclass=10):
        if nclass == 10:
            self.dataset = torchvision.datasets.CIFAR10(root=root, 
                                                        train=train,
                                                        download=download, 
                                                        transform=transform)
        elif nclass == 100:
            self.dataset = torchvision.datasets.CIFAR100(root=root, 
                                                        train=train,
                                                        download=download, 
                                                        transform=transform)
