        ...
        if len(param.shape) == 4:
            ...
            if update_param_grad.shape[0] > update_param_grad.shape[1]:
                ...
                model.average_grad[param_idx] = (1-args.alpha) * model.average_grad[param_idx] +\
                                                 args.alpha * (args.conv_cr * decoding_grad.reshape(sh))
                ...
                model.average_grad[param_idx] = (1-args.alpha) * model.average_grad[param_idx] +\
                                                 args.alpha * (args.conv_cr * update_param_grad.reshape(sh))
            ...
            # Compression Gradient with Random Matrix
            if param.grad.data.shape[0] > param.grad.data.shape[1]:
                encoding_grad = torch.mm(u_t, param.grad.data)
                decoding_grad = torch.mm(u, encoding_grad)
    
                model.average_grad[param_idx] = (1-args.alpha) * model.average_grad[param_idx] +\
                                                 args.alpha * (args.fc_cr * decoding_grad)
